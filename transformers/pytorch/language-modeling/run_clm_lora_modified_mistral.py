...
class MoeModelOutputWithPast(ModelOutput):
    ...
    safe_shift_gate: Optional[Tuple[torch.FloatTensor]] = None
...
class MoeCausalLMOutputWithPast(ModelOutput):
    ...
    attentions: Optional[Tuple[torch.FloatTensor, ...]] = None
    router_logits: Optional[Tuple[torch.FloatTensor]] = None
    safe_shift_gate_prediction: torch.FloatTensor = None
...
class MixtralDecoderLayer(nn.Module):
    def __init__(self, config: MixtralConfig, layer_idx: int):
        ...
        # self.safe_shift = nn.Parameter(torch.tensor(1, config.hidden_size))
        # self.safe_shift = nn.Parameter(torch.zeros(1, self.hidden_size))
    ...
    def forward(
            ...
            safe_shift_gate: torch.Tensor = None,
            **kwargs,
        ...
        Args:
            ...
            use_cache (`bool`, *optional*):
                ...
                (see `past_key_values`).
        """
        # hidden_states = (batch, seq_len, hidden_size), safe_shift_gate =(batch, seq_len), self.safe_shift=(1, hidden_size)
        # print(self.safe_shift.shape, safe_shift_gate.shape)

        # hidden_states = hidden_states # + torch.matmul(safe_shift_gate.unsqueeze(-1).bfloat16(), self.safe_shift)

        residual = hidden_states  # batch_size, seq_length, _
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def forward(
            ...
            safe_shift_gate: torch.LongTensor = None,
        ...
        hidden_states = inputs_embeds  # batch_size, seq_length, _
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    safe_shift_gate=safe_shift_gate,
                    use_cache=use_cache,
                )
        ...
        # safe_shift_feature = self.safe_shift_act_fn(self.safe_shift_w1(hidden_states)) * self.safe_shift_w3(
        #     hidden_states)
        # safe_shift_feature = self.safe_shift_w2(safe_shift_feature)
        # safe_shift_feature = safe_shift_feature.squeeze(-1)
        safe_shift_feature = None # self.safe_gate_classifier(hidden_states)[:, :, 7248]
        ...
        if not return_dict:
            return tuple(
                v
                for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_router_logits, safe_shift_feature]
                if v is not None
            )
        return MoeModelOutputWithPast(
            ...
            safe_shift_gate=safe_shift_feature,
        )
...
class MyMixtralForCausalLM(MixtralPreTrainedModel):
    ...
    def forward(
        ...
        """binary_safe = safe_labels
        safe_labels = torch.ones_like(copy.deepcopy(labels))
        for bs, sl, label in zip(binary_safe, safe_labels, labels):
            if bs == 0:
                sl.zero_()"""

        # print("👼"*10, "safe labels: {}\n\nlabels: {}".format(safe_labels[0], labels[0]))
        ...
        safe_shift_feature = outputs[-1]
        safe_shift_gate_prediction = None #nn.functional.sigmoid(safe_shift_feature)

        # 23166 Sorry
        if labels is not None:
            # TODO warning, the input prompt's safe gate should not be considered
            """safe_labels = safe_labels.to(safe_shift_feature.device)

            safe_labels = safe_labels[:, 1:].contiguous()
            safe_shift_feature = safe_shift_feature[:, :-1].contiguous()

            mask_matrix = labels[..., 1:] == -100
            mask_matrix = mask_matrix.to(safe_shift_feature.device)

            safe_shift_feature[mask_matrix] = 0
            safe_labels[mask_matrix] = 0"""

            # print("😄\n"*10, safe_shift_feature.shape, safe_labels.shape)
            # loss_safe_fct = nn.BCEWithLogitsLoss()
            # loss_safe = loss_safe_fct(safe_shift_feature, safe_labels.bfloat16())
            # print(loss_safe)
            # loss = loss_safe*0.2  # None
            # print(loss)
            ...
            ord_loss = loss_fct(shift_logits, shift_labels)
            loss = ord_loss
        ...
        if not return_dict:
            ...
            return (loss,) + output + (safe_shift_gate_prediction) if loss is not None else output + (safe_shift_gate_prediction)
        ...
        return MoeCausalLMOutputWithPast(loss=loss,
                                         ...
                                         safe_shift_gate_prediction=safe_shift_gate_prediction
                                         )
...
def load_dataset(data_files):
    ...
    temp["column_names"] = Dataset.from_dict({"train": ["text", "prefix", "safe"], "validation": ["text", "prefix", "safe"]})
...
def main():
    ...
    print("🌺 parameter load to my model finish"*10)
        ...
        with CaptureLogger(tok_logger) as cl:
            ...
            if "prefix" in column_names:
                ...
                if "safe" in column_names:
                    for label, prefix_len, safe_gate, safe_label in zip(labels, prefix_lengths, safe_gates, safe_labels):  # Do not compute loss for prompt inputs
