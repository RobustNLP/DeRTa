...
# Copied from run_files.models.mistral.modeling_mistral.MistralModel with MISTRAL->MIXTRAL,Mistral->Mixtral
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(
                v
                for v in
                [hidden_states, next_cache, all_hidden_states, all_self_attns, all_router_logits, safe_shift_feature]
                if v is not None
            )
...
class MyMixtralForCausalLM(MixtralPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from run_files import AutoTokenizer, MixtralForCausalLM
        ...
        if not return_dict:
            ...
            return (loss,) + output + (safe_shift_gate_prediction) if loss is not None else output + (
                safe_shift_gate_prediction)
